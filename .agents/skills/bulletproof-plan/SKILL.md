...
1. Read the issue: `gh issue view <number> --comments`. Capture title, body, and comments.
...
3. Read the grounding docs so your specialist briefs are precise:
   - `CLAUDE.md` — the project's standing guidance. Load-bearing for planning: **always verify your work** (`pnpm lint` and `pnpm build` are the minimum bar for "done" — `next build` also type-checks the project; run `npx tsc --noEmit` for a faster standalone type-check), **test proactively** — no test runner is wired up yet, so when a slice adds logic worth testing (data transforms, the blog indexer, tRPC/server procedures, `lib` utilities) the plan should stand up a runner (Vitest suits this TS stack) and colocate the tests rather than defer them; **never disable a rule to make a check pass**, **RTFM** (check the Next.js/React/Prisma/tRPC docs while planning to guard against version drift and hallucinated APIs), and **delight the user**.
   - `CONTEXT.md` — the binding glossary, **if it exists**. If it (or `CONTEXT-MAP.md`) is absent, **proceed silently** — don't flag the absence or propose creating it upfront. When present, note which domain terms the issue touches and use them verbatim.
...
4. Locate the code the issue touches (Glob/Grep) so briefs can cite real `file:line` anchors. Routes and pages live under `src/app/` (App Router — each blog post is its own `src/app/blog/<slug>/page.tsx`), shared UI under `src/components/`, server/tRPC procedures under `src/server/`, utilities under `src/lib/`, data and content metadata under `src/data/`, scripts like the blog indexer under `src/scripts/`, hooks under `src/hooks/`, and the database schema under `prisma/`.
...
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Senior engineer       | `general-purpose` (or `Frontend Developer` for component-heavy work)                  | Implementation path, edge cases, failure modes, and **test/verification strategy**: with no runner configured yet, how to keep logic testable (pull data transforms, the blog indexer, tRPC/server procedures, and `lib` utilities into plain modules), when to stand up a runner + first tests, and how else to verify — e.g. in the running app via `pnpm dev`. |
| Software architect    | `Software Architect` (or `Backend Architect` for data/tRPC-heavy work)                                                                     | System fit: Next.js App Router route boundaries (server vs client components), React component structure, tRPC/server-procedure design, the Prisma data model, and auth. Pattern choices, trade-offs, ADR alignment. |
...
2. **Flag ADR conflicts explicitly**: _"Contradicts ADR-XXXX (title) — but worth reopening because…"_ Never silently override an ADR.
...
6. **Performance and SEO check.** For a Next.js blog project, verify the plan doesn't regress page load times, doesn't break image optimization, and respects metadata/OG tag strategy.

### Phase 3 — Present to build

Write the finalized plan to the plan file, then call `ExitPlanMode` to gate on approval before implementing (it reads the plan from the file — no plan content is passed as an argument). Pre-declare the build commands you'll need as `allowedPrompts` (e.g. `pnpm lint`, `pnpm build`, `npx tsc --noEmit` for a standalone type-check, plus `pnpm db:generate` if the slice touches the Prisma schema, `pnpm blog:index` if it touches blog content, and `pnpm test` once a runner exists) to cut prompts later.
...
3. **When acceptance criteria are met — run the checks, commit, and kick off review.** All work must pass `pnpm lint` and `pnpm build` (the project's bar for "done" — `next build` type-checks the project; use `npx tsc --noEmit` for a faster standalone pass), plus any tests the slice added — never disable a rule to make a check pass. Commit the work, push the branch, then run `/code-review` for an automated review of the diff. For a deeper pass on larger changes, note that the user can trigger `/code-review ultra` (multi-agent cloud review) — you can't launch it yourself.
...
- **Tests are part of the slice, not after it.** Keep testable logic (data transforms, the blog indexer, tRPC/server procedures, `lib` utilities) in plain modules and write tests alongside the code — verified to catch a real bug (break the code, confirm the test fails, fix). No runner is configured yet, so a slice that needs tests stands one up as part of the work rather than punting it. Reviews run concurrently: self-review never blocks on the automated `/code-review`.
- **All work passes the checks.** `pnpm lint` and `pnpm build` are green (plus any tests the slice added) unless the user explicitly overrides — and a rule is never disabled to make a check pass.
...
- **Logic lives in the right place.** The plan places logic deliberately — server vs client components, tRPC/server procedures, `lib` utilities, and the Prisma data model — respects the App Router's server/client boundary, and aligns with any ADR the project keeps.
- Honors `CLAUDE.md` — especially proactive testing, RTFM while planning (Next.js/React/Prisma/tRPC docs), delight, and never disabling a lint/type rule to make it pass.
- Treats imported/user-authored content (contact-form input, external API data such as Bluesky/atproto responses, blog/MDX content) as untrusted.
