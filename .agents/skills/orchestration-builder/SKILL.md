...
- **branch slug**: derive a short kebab-case slug from the title if one is not given (e.g. `#128` "Add RSS feed to the blog index" becomes `128-blog-rss-feed`).
...
- **planning step**: default the `/bulletproof-plan` skill for per-item planning (if available). This is pluggable; if the user has no such skill, substitute "produce a written implementation plan with explicit review and verification steps."
...
```
#10 Add a `tags` field to the Prisma blog schema   blocked by: none
#11 Contact-form spam protection                   blocked by: none
#12 Tag filtering on the blog index page           blocked by: #10, #11
#13 Tag-based RSS feeds                             blocked by: #12
#14 Tag chips in the blog post header              blocked by: #10
```

Derived waves:

- Wave 1 (parallel): #10, #11 (no deps)
- Wave 2 (parallel): #12 (needs #10, #11), #14 (needs #10 only, so it joins Wave 2 rather than being serialized to the end)
- Wave 3: #13 (needs #12)

Note #14 lands in Wave 2 even though a human-written serial path might place it last. That is the parallelism the wave derivation is meant to surface. #12 and #14 would both be flagged for a shared-surface check if they touch the same blog components or data accessors.
