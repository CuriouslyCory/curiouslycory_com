...
4. **Verification gate (must pass before merge).** In the item's worktree, run this repo's minimum bar: `pnpm lint` clean and `pnpm build` green (`next build` type-checks the project; use `npx tsc --noEmit` for a faster standalone type-check), plus `pnpm db:generate` if the item touched the Prisma schema and any tests once a runner exists, and the item's acceptance criteria met. Honor any item-specific budget or guard noted in the plan.
...
# run pnpm lint + pnpm build on integration (npx tsc --noEmit for a standalone type-check); fix any merge fallout before continuing
git push
...
1. Run the full `pnpm lint` + `pnpm build` bar one final time on `{{INTEGRATION_BRANCH}}` (`npx tsc --noEmit` for a standalone type-check).
...
```
git switch {{INTEGRATION_BRANCH}} && git pull
# run pnpm lint + pnpm build on integration (npx tsc --noEmit for a standalone type-check); fix any fallout before the next wave
git worktree remove ../wt-<item>
```
...
1. Run the full `pnpm lint` + `pnpm build` bar on `{{INTEGRATION_BRANCH}}` (`npx tsc --noEmit` for a standalone type-check).
